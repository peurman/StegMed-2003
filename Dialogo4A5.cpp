...
CDialogo4A5::CDialogo4A5(CWnd* pParent /*=NULL*/)
	: CDialog(CDialogo4A5::IDD, pParent)
{
	//{{AFX_DATA_INIT(CDialogo4A5)
	m_Conclusion = _T("");
	m_Doctor = _T("Dr(a).");
	m_Edad = _T("");
	m_Fecha = _T("");
...
void CDialogo4A5::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDialogo4A5)
	...
	DDX_Text(pDX, IDC_TECNICA, m_Tecnica);
	//}}AFX_DATA_MAP
}


BEGIN_MESSAGE_MAP(CDialogo4A5, CDialog)
	//{{AFX_MSG_MAP(CDialogo4A5)
	ON_BN_CLICKED(IDC_SEGUIR4A5, OnSeguir4a5)
	ON_BN_CLICKED(IDC_ATRAS4A5, OnAtras4a5)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CDialogo4A5 message handlers

BOOL CDialogo4A5::OnInitDialog() 
{
	CDialog::OnInitDialog();
	
	// asociar la fuente al texto que la va a usar
	GetDlgItem(IDC_TEXTO4A5)->SetFont(&m_font1, TRUE);
	
	// configurar los botones  
	VERIFY(m_seguir.AutoLoad(IDC_SEGUIR4A5, this));
	VERIFY(m_atras.AutoLoad(IDC_ATRAS4A5, this));
	
	return TRUE; 
}

// Abre el di�logo siguiente 
void CDialogo4A5::OnSeguir4a5() 
{
	// organizar y cargar datos en m_Info  
	UpdateData(CTRL2VAR);
	CDialogo5A dlg5A(this);
	dlg5A.m_Info = "RESONANCIA MAGNETICA\r\n\r\nNombre completo:  " + m_Nombre + "        -        Fecha:  " + m_Fecha + "\r\nEdad:  " + m_Edad + " a�os" + "\r\n\r\nRegi�n:  " + m_Region + "\r\n\r\nT�cnica:\r\n     " + m_Tecnica + "\r\n\r\nInforme:\r\n     " + m_Informe + "\r\n\r\nConclusi�n:\r\n     " + m_Conclusion + "\r\n\r\nDoctor:\r\n   " + m_Doctor ;
	// abrir Dialogo5A 
	dlg5A.DoModal ();	
}

// Vuelve al di�logo anterior 
void CDialogo4A5::OnAtras4a5() 
{
	// volver al Dialogo3A
	EndDialog(IDCANCEL);		
}
